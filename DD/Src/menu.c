...
/* PD CONTROL CHANGE START: Add SET_PD_TD as the sixth motor-menu selection. */
char menu_sel[ROW][COLUMN][12] = {
    {"MAX_MIN_", "__4095__", "SEN_127_", "POSCHECK", "TURNMARK"},
    {"Set_VELO", "Set_ACC_", "END_ACC_", "HAND_ACC", "HAND_DEC", "SET_PD_TD"},
    {"1st_RACE", "2nd_RACE", "TURNDIST", "", ""}
};

static const uint8_t menu_item_count[ROW] = {5U, 6U, 3U};

void (* menu_functions[ROW][COLUMN])(void) =
{
    {F_Max_min,    F_4095,         F_127,        F_POSCHECK,   F_TURNMARK},
    {F_VELOCITY,   F_ACCELERATION, F_ENDACCEL,   F_HANDLE_ACC, F_HANDLE_DEC, F_PD_TD},
...
void menu(void)
{
    ...
    char scroll_buf[40];
    /* PD CONTROL CHANGE START: Expand the selection marker buffer for the sixth motor menu item. */
    char dots[14];
    /* PD CONTROL CHANGE END */
