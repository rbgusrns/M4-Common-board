...
void Race_Init()
{
    ...
    read_handle_dec_rom();
    read_turn_dist_rom();
    /* PD CONTROL CHANGE START: Load the saved Td value and apply it to the runtime PD controller. */
    read_pd_td_rom();
...
int race_stop_check( void )
{
    ...
    if( (RMotor.fp32next_vel <= 0.0f) && (LMotor.fp32next_vel <= 0.0f) )
    {
        /* 300 ms is long enough to settle after the end deceleration while
           avoiding an unnecessarily long stationary coil-current interval. */
        Motor_StartHold(300U);
        g_Flag.stop_check = OFF;  /* prevent a second-race hold from repeating */
...
void search_race()
{
    /* PD CONTROL CHANGE START: Reset PD state and synchronize the first-run controller to sensor frames. */
    uint32_t last_sensor_frame_seq = g_u32_sensor_frame_seq;
    PD_ControlReset();
    /* PD CONTROL CHANGE END */
    ...
    while(1)
    {
        /* PD CONTROL CHANGE START: Update position and PD steering once per complete sensor frame. */
        if (g_u32_sensor_frame_seq != last_sensor_frame_seq)
        {
            last_sensor_frame_seq = g_u32_sensor_frame_seq;
            make_position();
            PD_ControlUpdate();
            Handle();
        }
