...
position_t       g_pos;
/* PD CONTROL CHANGE START: Allocate persistent state for control-position PD steering. */
pd_control_t     g_pd_control;
...
/* PD CONTROL CHANGE START: Count complete sensor frames for deterministic PD updates. */
volatile uint32_t g_u32_sensor_frame_seq = 0;
/* PD CONTROL CHANGE END */
...
/* PD CONTROL CHANGE START: Store the menu and EEPROM representation of Td. */
uint16_t g_u16_pd_td = 0;
/* PD CONTROL CHANGE END */
volatile uint8_t g_motor_hold = OFF;
...
void Variable_Init(void)
{
    ...
    /* PD CONTROL CHANGE START: Initialize Td to the menu-scale default before PD controller setup. */
