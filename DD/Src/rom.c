...
void maxmin_write_rom( void )
{
    ...
    for (int i = 0; i < 16; i++) {
        uint32_t val = (uint32_t)(g_sen[i].fp32_4095_min_value);
        write_buf[ j++ ] = (uint16_t)((val >> 0) & 0xff);
        write_buf[ j++ ] = (uint16_t)((val >> 8) & 0xff);
    }
    for (int i = 0; i < 16; i++) {
        uint32_t val = (uint32_t)(g_sen[i].fp32_4095_max_value);
        write_buf[ j++ ] = (uint16_t)((val >> 0) & 0xff);
        write_buf[ j++ ] = (uint16_t)((val >> 8) & 0xff);
    }
...
void maxmin_read_rom( void )
{
    ...
    for (int i = 0; i < 16; i++) {
        ...
        g_sen[i].fp32_4095_min_value = (float)val;
    }
    for (int i = 0; i < 16; i++) {
        uint32_t val = 0;
        val = ((uint32_t)(read_buf[ j++ ] & 0xff) << 0);
        val |= ((uint32_t)(read_buf[ j++ ] & 0xff) << 8);
        g_sen[i].fp32_4095_max_value = (float)val;
    }
}
...
void write_mark_cnt_rom( void )
{
    ...
    mark_sarr[0] = (((uint16_t)g_u16_turnmark_cnt - 1) >> 0) & 0xff;
    mark_sarr[1] = (((uint16_t)g_u16_turnmark_cnt - 1) >> 8) & 0xff;
...
void read_mark_cnt_rom( void )
{
    ...
    g_u32_total_cnt = (int32_t)((mark_larr[0] & 0xff) << 0);
    g_u32_total_cnt |= (int32_t)((mark_larr[1] & 0xff) << 8);
}
...
void write_line_info_rom(void)
{
    ...
    for(i=0; i<=g_u16_turnmark_cnt; i++)
    {
...
void write_acc_handle_rom()
{
    uint16_t i = 0;
    uint16_t save_acc[8] = {0};
    save_acc[i++] = ((g_u16_handle_acc >> 0) & 0xff);
    save_acc[i++] = ((g_u16_handle_acc >> 8) & 0xff);
...
void read_handle_acc_rom()
{
    ...
    g_u16_handle_acc = ((lead_acc[i++] & 0xff) << 0);
    g_u16_handle_acc |= ((lead_acc[i++] & 0xff) << 8);
}
...
void write_dec_handle_rom()
{
    ...
    save_dec[i++] = ((g_u16_handle_dec >> 0) & 0xff);
    save_dec[i++] = ((g_u16_handle_dec >> 8) & 0xff);
...
void read_handle_dec_rom()
{
    ...
    g_u16_handle_dec = ((lead_dec[i++] & 0xff) << 0);
    g_u16_handle_dec |= ((lead_dec[i++] & 0xff) << 8);
}

/* PD CONTROL CHANGE START: Save and load the bounded integer Td value in its own EEPROM page. */
void write_pd_td_rom(void)
{
    uint16_t save_td[8] = {0};

    save_td[0] = (g_u16_pd_td >> 0) & 0xffU;
    save_td[1] = (g_u16_pd_td >> 8) & 0xffU;
    SpiWriteRom((uint16_t)PD_TD_PAGE, 0x00, 8, save_td);
}

void read_pd_td_rom(void)
{
    uint16_t load_td[8] = {0};

    SpiReadRom((uint16_t)PD_TD_PAGE, 0x00, 8, load_td);
    g_u16_pd_td = (load_td[0] & 0xffU);
    g_u16_pd_td |= (uint16_t)((load_td[1] & 0xffU) << 8);

    if (g_u16_pd_td > PD_TD_MAX_VALUE) {
        g_u16_pd_td = PD_TD_DEFAULT_VALUE;
    }
}
/* PD CONTROL CHANGE END */

void write_turn_dist_rom(void)
{
    uint16_t save_turn_dist[8] = {0};

    save_turn_dist[0] = (g_u16turn_dist >> 0) & 0xffU;
    save_turn_dist[1] = (g_u16turn_dist >> 8) & 0xffU;
    SpiWriteRom((uint16_t)TURN_DIST_PAGE, 0x00, 8, save_turn_dist);
}

void read_turn_dist_rom(void)
{
    uint16_t load_turn_dist[8] = {0};
    uint16_t saved_turn_dist;

    SpiReadRom((uint16_t)TURN_DIST_PAGE, 0x00, 8, load_turn_dist);
    saved_turn_dist = (load_turn_dist[0] & 0xffU);
    saved_turn_dist |= (uint16_t)((load_turn_dist[1] & 0xffU) << 8);

    /* Preserve the startup default when EEPROM is blank or contains invalid data. */
    if ((saved_turn_dist >= 1U) && (saved_turn_dist <= 1000U)) {
        g_u16turn_dist = saved_turn_dist;
    }
}
