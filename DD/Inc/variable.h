...
#define L_LED_ON   (L_LED_GPIO_Port->BSRR = L_LED_Pin)
#define L_LED_OFF  (L_LED_GPIO_Port->BSRR = ((uint32_t)L_LED_Pin << 16U))
#define R_LED_ON   (R_LED_GPIO_Port->BSRR = R_LED_Pin)
#define R_LED_OFF  (R_LED_GPIO_Port->BSRR = ((uint32_t)R_LED_Pin << 16U))
#define BLUE_ON
#define BLUE_OFF
...
/* PD CONTROL CHANGE START: Define menu storage units and safe range for the Td tuning value. */
#define PD_TD_STORAGE_SCALE   0.000001f
#define PD_TD_DEFAULT_VALUE   300U
#define PD_TD_MAX_VALUE       1000U
#define PD_TD_STEP            10U
...
/* PD CONTROL CHANGE START: Export the shared PD steering controller state. */
extern pd_control_t     g_pd_control;
/* PD CONTROL CHANGE END */
...
/* PD CONTROL CHANGE START: Export a counter that marks each complete sensor scan frame. */
extern volatile uint32_t g_u32_sensor_frame_seq;
/* PD CONTROL CHANGE END */
/* Set while the last valid stepper phase pattern is intentionally held. */
extern volatile uint8_t  g_motor_hold;
...
/* PD CONTROL CHANGE START: Export the EEPROM-backed integer representation of Td. */
extern uint16_t g_u16_pd_td;
/* PD CONTROL CHANGE END */
