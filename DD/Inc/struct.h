...
/* PD CONTROL CHANGE START: Add state and tuning values for filtered position-derivative steering. */
typedef struct
{
    float fp32prev_position;
    float fp32raw_position_rate;
    float fp32filtered_rate;
    float fp32control_position;
    float fp32td;
    float fp32filter_alpha;
    float fp32rate_limit;
    uint8_t u8initialized;
    uint8_t u8position_valid;
    uint8_t u8cross_active;
} pd_control_t;
